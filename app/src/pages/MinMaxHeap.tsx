...
export const MinMaxHeapPage: React.FC = () => {
  const { t } = useTranslation(['common', 'minmax'], { useSuspense: false });
  ...
  useEffect(() => {
      if (activeSteps.length > 0 && currentStepIdx >= 0) {
          ...
          // Convert string IDs "node-X" back to indices
  ...
  const handleExtractMin = () => {
      if (snapshot.length > 0) startOperation('Extract Min', heap.extractMin());
  };

  const handleExtractMax = () => {
      if (snapshot.length > 0) startOperation('Extract Max', heap.extractMax());
  };
          ...
          return (
              <div className="space-y-4 animate-in fade-in">
                  <div className="p-4 bg-slate-800 rounded-xl border border-slate-700">
                      <h3 className="text-white font-bold mb-1">Min-Max Heap Course</h3>
                      <p className="text-xs text-slate-400">Master the alternating levels of a Min-Max Heap.</p>
                  </div>
              ...
              <div className="flex-grow overflow-y-auto p-4 font-mono text-[10px] text-slate-300 custom-scrollbar">
                  ...
                  {activeTab === 'wiki' && (
                    <div className="space-y-8 font-sans pb-12 text-slate-400">
                      ...
                      <section><h4 className="text-blue-400 font-bold text-sm mb-2 uppercase">{t('minmax:wiki.concept')}</h4><SimpleMarkdown text={t('minmax:wiki.conceptDesc')} /></section>
                      <section><h4 className="text-green-400 font-bold text-sm mb-2 uppercase">{t('minmax:wiki.levels')}</h4><SimpleMarkdown text={t('minmax:wiki.levelsDesc')} /></section>
                      <section><h4 className="text-amber-400 font-bold text-sm mb-2 uppercase">{t('minmax:wiki.insert')}</h4><SimpleMarkdown text={t('minmax:wiki.insertDesc')} /></section>
                      <section><h4 className="text-red-400 font-bold text-sm mb-2 uppercase">{t('minmax:wiki.extract')}</h4><SimpleMarkdown text={t('minmax:wiki.extractDesc')} /></section>
                    </div>
                  )}
            ...
            <ControlIsland label="Operations" className="w-full lg:w-[320px]">
                <div className="grid grid-cols-[1fr_auto] gap-2 w-full">
                    ...
                    <div className="flex gap-1">
                        <button onClick={handleExtractMin} disabled={isPlaying} className="px-3 py-2.5 bg-blue-500 text-white rounded-xl text-[9px] font-black shadow-lg flex flex-col items-center justify-center gap-0.5 active:scale-95 transition-all shrink-0 w-16">
                            <ArrowDownToLine size={12} /> EXTRACT MIN
                        </button>
                        <button onClick={handleExtractMax} disabled={isPlaying} className="px-3 py-2.5 bg-red-500 text-white rounded-xl text-[9px] font-black shadow-lg flex flex-col items-center justify-center gap-0.5 active:scale-95 transition-all shrink-0 w-16">
                ...
                <div className="flex items-center gap-1 overflow-x-auto pb-2 custom-scrollbar no-pan h-full">
                    {snapshot.map((val, idx) => (
                        <div key={idx} className={`flex flex-col items-center shrink-0 group ${highlightedIndices.includes(idx) ? 'scale-110' : ''} transition-all`}>
                            <div className={`w-8 h-8 flex items-center justify-center rounded border text-xs font-bold ${highlightedIndices.includes(idx) ? 'bg-amber-100 border-amber-500 text-amber-900' : Math.floor(Math.log2(idx+1)) % 2 === 0 ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
                                {val}
                            </div>
                            <span className="text-[9px] text-slate-500 font-mono font-bold">{idx}</span>
                        </div>
                    ))}
                    {snapshot.length === 0 && <span className="text-xs text-slate-400 italic px-2">Empty Heap</span>}
                </div>
            </ControlIsland>
        </div>
      </div>
