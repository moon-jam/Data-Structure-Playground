...
export const DeapPage: React.FC = () => {
  const { t } = useTranslation(['common', 'deap'], { useSuspense: false });
  const [heap] = useState(() => new DEAP());
  const [snapshot, setSnapshot] = useState<DeapSnapshot>([null, null]);
  
  // Playback State
  const [history, setHistory] = useState<{id: string, action: string, steps: VisualizationStep[], finalSnapshot: DeapSnapshot}[]>([]);
          ...
          return (
              <div className="space-y-4 animate-in fade-in">
                  <div className="p-4 bg-slate-800 rounded-xl border border-slate-700">
                      <h3 className="text-white font-bold mb-1">DEAP Course</h3>
                      <p className="text-xs text-slate-400">Learn about Double-ended Heaps.</p>
                  </div>
              ...
              <div className="flex-grow overflow-y-auto p-4 font-mono text-[10px] text-slate-300 custom-scrollbar">
                  ...
                  {activeTab === 'wiki' && (
                    <div className="space-y-8 font-sans pb-12 text-slate-400">
                      ...
                      <section><h4 className="text-blue-400 font-bold text-sm mb-2 uppercase">{t('deap:wiki.concept')}</h4><SimpleMarkdown text={t('deap:wiki.conceptDesc')} /></section>
                      <section><h4 className="text-green-400 font-bold text-sm mb-2 uppercase">{t('deap:wiki.structure')}</h4><SimpleMarkdown text={t('deap:wiki.structureDesc')} /></section>
                      <section><h4 className="text-amber-400 font-bold text-sm mb-2 uppercase">{t('deap:wiki.insert')}</h4><SimpleMarkdown text={t('deap:wiki.insertDesc')} /></section>
                      <section><h4 className="text-red-400 font-bold text-sm mb-2 uppercase">{t('deap:wiki.extract')}</h4><SimpleMarkdown text={t('deap:wiki.extractDesc')} /></section>
                    </div>
                  )}
                            ...
                            <div className="w-[4000px] h-[4000px] relative cursor-grab active:cursor-grabbing">
                                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ overflow: 'visible' }}>
                                    <div className="absolute" style={{ left: 0, top: 0 }}>
                                      <DeapNode value={null} index={1} highlightedIndices={[]} />
                                    </div>
                ...
                <div className="flex items-center gap-1 overflow-x-auto pb-2 custom-scrollbar no-pan h-full">
                    {snapshot.map((val, idx) => idx > 1 && (
                        <div key={idx} className={`flex flex-col items-center shrink-0 group ${highlightedIndices.includes(idx) ? 'scale-110' : ''} transition-all`}>
                            <div className={`w-8 h-8 flex items-center justify-center rounded border text-xs font-bold ${highlightedIndices.includes(idx) ? 'bg-amber-100 border-amber-500 text-amber-900' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                                {val}
                            </div>
