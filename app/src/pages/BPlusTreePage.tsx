...
export const BPlusTreePage: React.FC = () => {
  const { t } = useTranslation(['common', 'bplustree'], { useSuspense: false });
  const [order, setOrder] = useState(3);
  const [tree, setTree] = useState(() => new BPlusTree(3));
  const [snapshot, setSnapshot] = useState<BPlusTreeSnapshot>(null);
  
  const [history, setHistory] = useState<{id: string, action: string, steps: VisualizationStep[], finalSnapshot: BPlusTreeSnapshot}[]>([]);
          ...
          return (
              <div className="space-y-4 animate-in fade-in">
                  <div className="p-4 bg-slate-800 rounded-xl border border-slate-700">
                      ...
                      <p className="text-xs text-slate-400">Master the leaf-linked variation of B-Tree.</p>
                  </div>
              ...
              <div className="flex-grow overflow-y-auto p-4 font-mono text-[10px] text-slate-300 custom-scrollbar">
                  ...
                  {activeTab === 'wiki' && (
                    <div className="space-y-8 font-sans pb-12 text-slate-400">
                      ...
                      <section><h4 className="text-blue-400 font-bold text-sm mb-2 uppercase">{t('bplustree:wiki.concept')}</h4><SimpleMarkdown text={t('bplustree:wiki.conceptDesc')} /></section>
                      <section><h4 className="text-green-400 font-bold text-sm mb-2 uppercase">{t('bplustree:wiki.properties')}</h4><SimpleMarkdown text={t('bplustree:wiki.propertiesDesc')} /></section>
                      <section><h4 className="text-purple-400 font-bold text-sm mb-2 uppercase">{t('bplustree:wiki.insertion')}</h4><SimpleMarkdown text={t('bplustree:wiki.insertionDesc')} /></section>
                      <section><h4 className="text-red-400 font-bold text-sm mb-2 uppercase">{t('bplustree:wiki.deletion')}</h4><SimpleMarkdown text={t('bplustree:wiki.deletionDesc')} /></section>
                      <section><h4 className="text-amber-400 font-bold text-sm mb-2 uppercase">{t('bplustree:wiki.applications')}</h4><SimpleMarkdown text={t('bplustree:wiki.applicationsDesc')} /></section>
                      <section><h4 className="text-cyan-400 font-bold text-sm mb-2 uppercase">{t('bplustree:wiki.complexity')}</h4><SimpleMarkdown text={t('bplustree:wiki.complexityDesc')} /></section>
                    </div>
                  )}
        ...
        <div className="flex-grow relative overflow-hidden bg-grid-slate-100">
             ...
             <TransformWrapper initialScale={1} minScale={0.1} maxScale={2} centerOnInit>
                {({ zoomIn, zoomOut, resetTransform }) => {
                    const { root: layoutRoot, positions } = calculateLayout(snapshot);
                    return (
                    <>
                        <TransformComponent wrapperStyle={{ width: "100%", height: "100%" }}>
                            <div className="w-[4000px] h-[4000px] relative cursor-grab active:cursor-grabbing">
                                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ overflow: 'visible' }}>
                                    {layoutRoot && <BPlusTreeNode node={layoutRoot} positions={positions} highlightedIds={highlightedIds} onKeyClick={handleKeyClick} />}
                                </div>
                            </div>
                        </TransformComponent>
